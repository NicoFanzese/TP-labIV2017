...
export class ServicioOfertasService {
  ...
  getDetalleProductosOferta(idOferta) {
    //console.log(this.rutaOfertaProductos +"?idOferta="+ idOferta);
      return this.http.get(this.rutaOfertaProductos +"?idOferta="+ idOferta).map(
      data => data.json());
  }
  ...
  public GuardarProductoOferta(productoOferta: ProductoOferta) 
  {
    //Configuro headers
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
